    ...
    <header className="sticky top-0 z-40 w-full border-b border-stone-200/80 bg-[#FAFAF7]/95 backdrop-blur-md transition-all">
          ...
          <div className="flex items-center gap-3 overflow-x-auto">
            <div className="flex items-center gap-2 font-medium text-stone-800">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#0E402D] opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-[#0E402D]"></span>
              </span>
              ...
              >
                <RefreshCw className={`w-3 h-3 ${rateLoading ? 'animate-spin text-[#0E402D]' : ''}`} />
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <Link to="/" className="flex items-center gap-3 shrink-0 group">
          <div className="w-9 h-9 rounded-lg bg-[#0E402D] flex items-center justify-center text-[#9FCC2E] shadow-xs group-hover:bg-[#295135] transition-colors border border-[#0E402D]">
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-2 px-3.5 py-2 rounded-lg text-xs font-semibold transition-all ${
                    isActive
                      ? 'bg-[#0E402D] text-white font-bold shadow-2xs'
              ...
              >
                ...
                {link.badge !== undefined && link.badge > 0 && (
                  <span className="ml-0.5 px-1.5 py-0.2 text-[10px] font-bold rounded-full bg-[#9FCC2E] text-[#0E402D] border border-[#0E402D]/20">
                    {link.badge}
                  </span>
                )}
              </NavLink>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Link
            to="/calculator"
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-[#0E402D] hover:bg-[#295135] text-white font-semibold text-xs sm:text-sm transition-all shadow-xs border border-[#0E402D]"
          >
          ...
          {navLinks.map((link) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
                    isActive
                      ? 'bg-[#0E402D] text-white font-semibold'
              ...
              >
                ...
                {link.badge !== undefined && link.badge > 0 && (
                  <span className="px-2 py-0.5 text-xs font-bold rounded-full bg-[#9FCC2E] text-[#0E402D]">
                    {link.badge}
                  </span>
                )}
              </NavLink>
